...
public class CreateEtudiantUseCase(IRepositoryFactory factoryRepository)
{
    ...
    public async Task<Etudiant> ExecuteAsync(Etudiant etudiant)
    {
        ...
        Etudiant et = await factoryRepository.EtudiantRepository().CreateAsync(etudiant);
        factoryRepository.EtudiantRepository().SaveChangesAsync().Wait();
        return et;
    }
    private async Task CheckBusinessRules(Etudiant etudiant)
    {
        ...
        ArgumentNullException.ThrowIfNull(factoryRepository.EtudiantRepository());
        
        // On recherche un étudiant avec le même numéro étudiant
        List<Etudiant> existe = await factoryRepository.EtudiantRepository().FindByConditionAsync(e=>e.NumEtud.Equals(etudiant.NumEtud));
        ...
        existe = await factoryRepository.EtudiantRepository().FindByConditionAsync(e=>e.Email.Equals(etudiant.Email));
